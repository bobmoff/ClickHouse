...
private:
    ...
    public:
        ...
        {
            LOG_WARNING(log, "flushing request OUT {}", bool(request_stream));
            if (request_stream)
            {
            ...
            LOG_TEST(log, "flushing request OUT {} : {}", bool(request_stream), request_stream_completed);
        ...
        ~PooledConnection() override
        {
            ...
            response_stream = nullptr;
            LOG_TEST(log, "flushing request IN {} : {}", bool(response_stream), request_stream_completed);

            LOG_TEST(log, "~Sessions {}. request_completed {}", Session::getHost(), request_stream_completed && response_stream_completed);

            if (request_stream_completed && response_stream_completed)
            {
                if (auto lock = pool.lock())
                {
                ...
                {
                    ProfileEvents::increment(metrics.reset, 1);
                }
            }
    ...
    private:
        ...
        void jumpToOtherConnection(PooledConnection & connection)
        {
            ...
            if (!buffer.empty())
                throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "jumpToOtherConnection with buffered data in src");
