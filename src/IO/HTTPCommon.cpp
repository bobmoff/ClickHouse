...
HTTPSessionPtr makePooledHTTPSession(
    const Poco::URI & uri,
    const ConnectionTimeouts & timeouts,
    size_t /*connection_pool_soft_limit*/,
    size_t /*connection_pool_warning_limit*/,
    ProxyConfiguration proxy_configuration)
{
    chassert(ConnectionPools::instance().isPoolDeclared(uri, proxy_configuration));
    return makeHTTPSession(uri, timeouts, std::move(proxy_configuration));
}
